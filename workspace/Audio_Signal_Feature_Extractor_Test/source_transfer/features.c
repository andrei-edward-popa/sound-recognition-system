...
void init_LLD(low_level_descriptors* features_ctx) {
	...
	features_ctx->mfcc = (float32_t**)malloc(COLLECT_TIME * sizeof(float32_t*));
...
void compute_LowLevelDescriptors_f32(low_level_descriptors* features_ctx, u32 time_index, arm_rfft_instance_f32 rfft_instance) {
...
#endif
#if defined PITCH
	compute_PitchHarmonicRatio_f32(features_ctx->x.samples, PITCH_EP, SAMPLE_RATE, FRAME_LENGTH / 2, &features_ctx->pitch[time_index], &features_ctx->harmonic_ratio[time_index]);
	if (time_index == COLLECT_TIME - 1) {
		float32_t mean, var;
		arm_var_f32(features_ctx->harmonic_ratio, COLLECT_TIME, &features_ctx->s_harmonicRatio);
		u32 k_pitch = 0;
		for (u32 i = 0; i < COLLECT_TIME; i++) {
...
#if defined PITCH
	if (time_index == COLLECT_TIME - 1) {
		arm_copy_f32(features_ctx->zero_crossing_rate, features_ctx->silence_crossing_rate, COLLECT_TIME);
		arm_mult_f32(features_ctx->spectral_rolloff, features_ctx->silence_crossing_rate, features_ctx->silence_crossing_rate, COLLECT_TIME);
		arm_mult_f32(features_ctx->harmonic_ratio, features_ctx->silence_crossing_rate, features_ctx->silence_crossing_rate, COLLECT_TIME);
		for (u32 i = 0; i < COLLECT_TIME; i++) {
			if (features_ctx->silence_crossing_rate[i] / features_ctx->s_harmonicRatio < SAD_THRESHOLD2) {
	...
	if (time_index == COLLECT_TIME - 1) {
		for (u32 i = 0; i < NUMBER_OF_CEPSTRALS; i++) {
			float32_t sum_mfcc = 0;
			for (u32 j = 0; j < COLLECT_TIME; j++) {
				sum_mfcc += features_ctx->mfcc[j][i];
			}
			features_ctx->m_MFCC[i] = sum_mfcc / COLLECT_TIME;
		}
	}
#endif
}
